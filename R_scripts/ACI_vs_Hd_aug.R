...
## Hd_aug is the observable variable
response_Hd_aug_dataframe <- 
  read.table(
    paste0(file_path, 
           "Optimal-Constraint-Data/Data_for_emulation/aug/CDNC_PPE_NH_SH_difference_aug_revised_match_MODIS_CF.dat"
           ), 
  col.names = "Hd_aug"
)
...
response_ACI_dataframe <- 
  read.table(
    paste0(file_path, 
           "./Optimal-Constraint-Data/Data_for_emulation/ann/ACI_PPE_global_ann.dat"
    ), 
    col.names = "ACI"
  )
...
## Next for Hd_aug
R2adj_Hd_aug <- summary(
  lm(Hd_aug ~., data.frame(cbind(response_Hd_aug_dataframe, inputs_x_norm_T_matrix)))
)$adj.r.squared
...
write_lines(R2adj_Hd_aug,
            file="objects/R2adj_Hd_aug.txt")
# R2adj_Hd_aug <-
#   as.numeric(readLines("objects/R2adj_Hd_aug.txt"))
...
## Next we'll fit a GPs for Hd_aug

gppdnormalised_t_Hd_aug_matrix <- gp_d_dxi_normalised_function(response_Hd_aug_dataframe)

# And now the alignment measure

mean(abs(colSums(gppdnormalised_t_ACI_matrix * gppdnormalised_t_Hd_aug_matrix)))