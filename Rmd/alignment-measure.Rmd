---
title: "Response surface alignment measure"
author: ""
date: ""
output: 
  pdf_document:
...
nugget_value <- if(perform_SA == "no"){1e-8}
...
```{r type-of-analysis-statement, echo=F}
...
```{r prim-so4-diam-statement, echo=F}
prim_so4_diam_statement <- ifelse(prim_so4_diam_removal == "yes",
                                  ...
                                  "")
if(prim_so4_diam_removal == "yes") {rm(prim_so4_diam_low_indices)} 
rm(prim_so4_diam_removal)
```

`r type_of_analysis_statement`. `r paste0(prim_so4_diam_statement)`

```{r remove-objects-1, echo=F}
rm(prim_so4_diam_statement, type_of_analysis_statement)
```

```{r map-of-selected-gridboxes, fig.cap="Selected gridboxes highlighted in green and blue.", echo=F}
...
```{r SA-1, fig.cap="Main effects for the first output.", echo=F}
if(perform_SA == "yes"){
...
```{r SA-2, fig.cap="Main effects for the second output.", echo=F}
if(perform_SA == "yes"){
...
```{r post-SA-inputs-and-objects, echo=F}
if(perform_SA == "yes"){
...
```{r sa-pie-charts, figures-side, fig.show="hold", fig.cap="The variables with main effects summing to at leaat 95% for (left) output 1 and (right) output 2.", out.width="50%", echo=F}
# library(ggplot2)
...
To fit the emulator, we assume a linear mean function of the form $$\hspace{2.6cm}  m(\textbf{x}) = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \dots + \beta_px_p.$$

```{r estimation-method, echo=F}
estimation_method <- ifelse(em == "post_mode", "maximum posterior mode estimation", "maximum likelihood estimation")
```

The Gauss (squared exponential) kernel is used for the covariance function, `r estimation_method` for the parameters estimates (trend, range, SD and the nugget term), and the `RobustGaSP` package to fit the emulators.

```{r remove-objects-2, echo=F}
if(perform_SA == "yes"){
...
```{r posterior-predictions, echo=F}
...
For validation of the emulators fitted, as well as calculation of the alignment measure, `r num_testing_input` new LHC points are generated and predictions made at these points. The mean width of the 95% CI's for these predictions are shown in Table 3, along with the mean width of the LOO CV prediction 95% CI's. Note that the range of output 1 is `r round(max(output1) - min(output1),1)` and of output 2 is `r round(max(output2) - min(output2),1)`.
...
```{r loo-cv-plots-0, figures-side, fig.show="hold", fig.cap="LOO CV validation plot for output 1 (left) and output 2 (right).", out.width="50%", echo=F}
...
```{r loo-cv-plots, figures-side, fig.show="hold", fig.cap="LOO CV validation plot for output 1 (left) and output 2 (right).", out.width="50%", eval=F, include=F}
par(mar = c(4, 4, .1, .1))
loo_validation_plot(loo_cv1)
loo_validation_plot(loo_cv2)
```
...
```{r by-hand-checks, fig.show="hold", out.width="50%", fig.cap="The predictions found by-hand match well with those made by the RobustGaSP package (left), with the latter minus the former found to be essentially zero (right). Top: Output 1. Bottom: Output 2.", echo=F}
par(mar = c(4, 4, .1, .1))
...
Ahead of performing by-hand calculations of the partial derivatives at the testing points, by-hand calculations for the predictions made at these points are plotted against the predictions returned by the `RobustGaSP` package (left hand plots in Figure 3) and the difference between the two plotted against the test input index (right hand plots in Figure 3). Finally, for both we calculate $$\dfrac{\sum\limits_{i=1}^{N}\Biggl|\text{(predictions from package)}_{i} - \text{(predictions by-hand)}_{i}\Biggr|}{N},$$ where $N$ is the number of test points 
<!-- , then divide this by $10^{-8}$  -->
<!-- (machine precision)  -->
to get `r signif(byhand_score1, 1)` for output 1 and `r signif(byhand_score2, 1)` for output 2.
...
Having calculated the emulated partial derivatives for use in the alignment measure, and normalising these, we calculate the AM, getting a value of `r round(AM,3)`.